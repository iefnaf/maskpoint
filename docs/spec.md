...
73b. As an agent, I want to recover the verbatim original behind any mask placeholder while I keep working, so that an exact error, path, or command is one tool call away instead of lost to the compaction.
...
- Pi ships as a pi package declaring its extension entrypoint. Claude Code and Codex ship as plugins bundling hook configuration plus the CLI entrypoint. DSH ships as an external package implementing its compaction seam, installed as a host bundle that replaces the built-in backend (decided by spike #9; see `docs/design.md` Open issue 1).
...
- Integration point is the host's compaction service seam: implement the same abstract service as the host's built-in backend and replace the built-in row in the host composition. One backend per context, so it replaces rather than joins the built-in.
...
- The masking path must land a model-free surface replacement that carries accurate shadow pricing so the host's replay-based token accounting stays correct. The host already defines a model-free replacement protocol for its tool-result pruner. Decided by spike #9: the automatic entry uses that protocol, landing per observation in place; the explicit entries land through the summary path with a Maskpoint envelope and no summarization-call marker, because the host's session invariant refuses in-place replacement between turns and `compactRegion` must return a compaction result. See `docs/design.md` Open issue 2.
