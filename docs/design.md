...
*Known limits of the masking-only landing.* Statistics are logged, not persisted (`persistMetadata: false`); the host's summary event has no field for `EngineDetail`. An explicit compaction with nothing worth masking fails with the host's `summary` error (a masked replacement could only grow), which is the honest outcome. If masking alone leaves the surface above the trigger, the backend logs it and does not compact further until #11 adds the checkpoint path. Images in user messages are noted as omitted rather than carried.
