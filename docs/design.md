...
The adapter implements the host's abstract compaction service and is selected by preset like any other backend. All three entry points are honoured: automatic trigger, explicit idle-session compaction, and explicit region compaction, with the seam's pairing predicates validating region edges.

The one genuinely undecided piece is the durable representation of a **model-free** masked replacement. The host already defines a model-free replacement protocol for its tool-result pruner, which carries shadow pricing so replay-based accounting stays correct; the alternative is the summary path, which requires a provider/model envelope and a summarization-call marker. Using the summary path for something no model wrote would misreport the session log. This is Open issue 2, and it must be settled before implementation because it determines the session event sequence, and therefore the host's token accounting and client rendering.
...
One sanitized fixture corpus drives both: directly for Seam 1, and through each host's recording for Seam 2, so cross-platform parity is asserted instead of assumed. Legitimate host differences are explicit expectations in the parity test, not silent exceptions. Checkpoint tests use a deterministic model double covering success, error, abort, length stop, tool call, and empty response. Sanitization is checked in CI. Host-dependent smoke tests are documented manual procedures, not CI requirements, because CI has no host binaries.

## Open issues

1. **DSH packaging — in-tree backend or external plugin.** Determines whether the adapter is a package inside that repository or one that registers a provider against the seam. Candidates: ship as an in-tree sibling of the built-in backend; ship as an external package if the loader permits providing the service. Next step: inspect how the seam resolves its provider and whether an external package can mount it.
2. **DSH model-free replacement event.** Candidates: reuse the prune replacement protocol with shadow pricing; emit the summary event with a provider/model envelope but no summarization-call marker. The second misreports the log, so it is a reluctant fallback. Next step: read the session and surface APIs and spike a mask-only compaction.
